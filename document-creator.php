...
function docrt_no_surat($type,$meta,$postID) {
    ...
    $no = '35.73'.'.'.docrt_dd('kkec');
    // Surat Utama 16

    $data['sku']    = '188.451/'.$meta['docrt_sku_id'][0].'/'.$no.'/'.get_the_date('Y',$postID) ;
...
function docrt_who_give_ttd($jenis_ttd) {
    ...
    foreach ($pejabat as $k => $v) {
        // Kondisi kusus
        $camat = strtoupper('Camat '.$data_dasar['kec']);
        switch ($v['kode']) {
        case 'camat':
            $option_jabatan = $camat;
            $option_kasi = '';
            break;
        case 'sekcam':
            $option_jabatan = $camat.'<br /><span style="font-size:11px;">'.$v['jabatan'].'</span>';
            $option_kasi = '';
            break;
        default:
            $option_jabatan = 'an: '.$camat.'<br /><span style="font-size:11px;">'.$v['jabatan'].'</span>';
...
function docrt_load_scripts_admin() {
    ...
    wp_enqueue_script( 'highchart-js', docrt_plugin_url() . '/assets/js/highcharts.js', array('jquery'), '' );
...
function docrt_get_terms_active() {
    $terms = array(
        'sku'   => 'keputusan camat '.get_option('docrt_data_dasar')['kec'].' kota malang',
...
include_once __DIR__ . '/includes/docrt_report_template.php';
