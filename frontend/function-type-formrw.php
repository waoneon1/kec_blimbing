...
function docrt_cpt_formrw() {
    ...
    register_post_type('docrt-formrw', array(
        ...
        'supports' => array( 'title', 'author'/*, 'comments', 'custom-fields', , 'publicize', 'wpcom-markdown'*/ ),
...
// HIDE ADD NEW POST ON RW FORM =======================================================
function disable_new_posts() {
    // Hide sidebar link
    global $submenu;
    unset($submenu['edit.php?post_type=docrt-formrw'][10]);

    // Hide link on listing page
    echo '<style type="text/css">
    .post-type-docrt-formrw #wpbody #wpbody-content .wrap a.page-title-action { display:none; }
    </style>';
}
...
function alter_primary_nav( $items, $args ) {
   // global $wp; add_query_arg( $wp->query_string, '', home_url( $wp->request ) )
    ...
    if (is_user_logged_in()) {
        $log = '<a href="'.wp_logout_url(home_url()).'" class="nav-link docrt-log">log out</a>';
    } else {
        $log = '<a href="'.wp_login_url().'" class="nav-link docrt-log">log in</a>';
    }
